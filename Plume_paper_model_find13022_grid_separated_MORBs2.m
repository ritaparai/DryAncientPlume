...
%% RParai Oct 20 2020
%% Nov 11 2020 time to make the MORBs work
...
ours = 5e8/10;                  %% spend more time searching in around the right ballpark
maximum = ours;
...
Nrange = linspace(4.2,5.2,30);
initrange = linspace(5e9,8e9,30);
...
%% choose a 130Xe/22Ne value for the MORB mantle
test_res = 0.02;
%% fraction 130Xe atm
PRf130a = 0.75;
adjustment = 1;
...
% He3Ne22acc = (He3sol + He3cc + He3atm) ./ Ne22M < PR322

% He3Ne22_afterMO = PR322 * (1 ./ (1-xMORB(3)));
% FrFr = 1.8;
...
plotx = [0.478 0.001; 1.104 3e-3; 0.1489 1e-4; 0.7763 7e-4; 0.4145 4e-4; 0.3643 4e-4];  %% PR 2PD43 using 40Ar/36Ar of 27000
...
BestXeidx = abs(store130fracs(2,:) - PRf130a) == min(abs(store130fracs(2,:) - PRf130a));

% test_res = abs(store130fracs(2,BestXeidx) - PRf130a);
% PRf130a = store130fracs(2,BestXeidx);
% test_res = 0.005;
% end
...
score(5,:) = ((store2022M(end,BestXeidx) - PR2022)./0.01).^2 + ((store2122M(end,BestXeidx) - targetNe(2))./0.0002).^2;
score(6,:) = ((store43M(end,BestXeidx) - mean(targetHe))./520).^2;
% if mean(store43M(end,closetwice)) > targetHe(1) && mean(store43M(end,closetwice)) < targetHe(2)
%     score(6,:) = 0;
% else
%     score(6,:) = 100;
% end
score(7,:) = ((store3130M(end,BestXeidx) - PR3130)./100).^2;
score(8,:) = ((store13022M(end,BestXeidx) - PR13022)./0.0005).^2;
score(9,:) = ((store322M(end,BestXeidx) - PR322)./0.1).^2;
% score(10,:) = ((store130fracs(2,BestXeidx) - 0.95)./0.02).^2;
...
% f130ares(grid1,grid2) = sum(score(10,bestscore));
        ...
        %% otherwise it just stays NaN
    end
end

save MORBgrid_acct12_f130a0p75_3He130Xe1000_Mres2e27_finer.mat
...
closetwice = false(size(closetwice));
...
plot(store3130M(end,itscloseidx),store43M(end,itscloseidx),'c.','MarkerSize',10);
...
plot(0.0590,12.6,'g*');
plot(0.029,9.8,'ks');
plot(store2122M(end,itscloseidx),store2022M(end,itscloseidx),'c.','MarkerSize',10);
% plot(store2122M(end,closetwice),store2022M(end,closetwice),'m.');
xlabel('21Ne/22Ne'); ylabel('20Ne/22Ne');
cz = 'k'; axis square; box on;

subplot(2,3,3)
plot(PR13022,PR322,'ms'); xlabel('130Xe/22Ne'); ylabel('3He/22Ne'); axis square; box on; hold on;
plot(store13022M(end,:),store322M(end,:),'k.');
plot(store13022M(end,itscloseidx),store322M(end,itscloseidx),'c.','MarkerSize',10);
...
scatter(store130132M(end,itscloseidx),store131132M(end,itscloseidx),10,'c','filled');
...
scatter(store130132M(end,itscloseidx),store134132M(end,itscloseidx),10,'c','filled');
% scatter(store130132M(end,closetwice),store134132M(end,closetwice),4,'m','filled');
xlim([0.145 0.165]); ylim([0.375 0.43]);
...
scatter(store130132M(end,itscloseidx),store136132M(end,itscloseidx),10,'c','filled');
% scatter(store130132M(end,closetwice),store136132M(end,closetwice),4,'m','filled');
xlim([0.145 0.165]); ylim([0.3 0.38]);

extable = [Nres Xe130i PRf130a PR13022 PR3130 Xe130Ne22 xMORB']';

figure;
subplot(1,2,1)
plot(t(startidx:end),store43M(:,BestXeidx),'b-');
xlabel('time'); ylabel('4He/3He'); axis square; box on; xlim([0 T]);
subplot(1,2,2); set(gca,'yscale','log')
plot(t(startidx:end),storeHe3M(:,BestXeidx),'b-');
...
figure;
subplot(2,2,1); 
errorbar(targetNe(2),PR2022,0.01, 0.01, 0.0002, 0.0002,'gs'); hold on;
plot(0.029,9.8,'ks');
plot(store2122M(:,BestXeidx),store2022M(:,BestXeidx),'m-');
plot(store2122M(1,BestXeidx),store2022M(1,BestXeidx),'ro');
xlabel('21Ne/22Ne'); ylabel('20Ne/22Ne');
axis square; box on; xlim([0.029 0.060]); ylim([9.7 13.4]);
...
plot([components(2,1,1) components(2,3,1)],[components(3,1,1) components(3,3,1)],'b-');
plot([components(2,1,1) components(2,4,1)],[components(3,1,1) components(3,4,1)],'r-');
plot(store130132M(:,BestXeidx),store131132M(:,BestXeidx),'m-');
xlim([0.145 0.165]); ylim([0.76 0.83]);
...
plot([components(2,1,1) components(2,3,1)],[components(4,1,1) components(4,3,1)],'b-');
plot([components(2,1,1) components(2,4,1)],[components(4,1,1) components(4,4,1)],'r-');
plot(store130132M(:,BestXeidx),store134132M(:,BestXeidx),'m-');
xlim([0.145 0.165]); ylim([0.375 0.43]);
...
plot([components(2,1,1) components(2,3,1)],[components(5,1,1) components(5,3,1)],'b-');
plot([components(2,1,1) components(2,4,1)],[components(5,1,1) components(5,4,1)],'r-');
plot(store130132M(:,BestXeidx),store136132M(:,BestXeidx),'m-');
xlim([0.145 0.165]); ylim([0.3 0.38]);
