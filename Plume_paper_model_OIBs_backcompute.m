...
plotx = [0.475 0.002; 1.032 8e-3; 0.1489 1e-4; 0.7823 1e-3; 0.4012 8e-4; 0.3460 1e-3];

%% =================================

%% Start with the parameter ranges
Nrange = linspace(1,4.2,100);
initrange = linspace(1e8,4e9,100);
...
XeR130 = (Icef130a * Xe130now)./(1-Icef130a);
...
score(5,:) = ((store2022M - Ice2022)./0.02).^2 + ((store2122M - targetNe(2))./0.0003).^2;
score(6,:) = ((store43M - mean(targetHe))./3000).^2;
score(7,:) = ((store3130M - Ice3130)./100).^2;
score(8,:) = ((store13022M - Ice13022)./0.0005).^2;
score(9,:) = ((store322M - Ice322)./0.1).^2;
...
% save OIBgrid_10212021_acct18_f130a0p95_3He130Xe800_Mres2e27_no128w131_fine_forS4.mat
